...
the look, you can apply a theme, and a color scheme. Let's take a look at themes first by using the
...
You could modify every feature of the theme like that, but in some cases it might be easier to
choose one from the `ggthemes` package:
...
For example in the code above, I have used `theme_minimal()` which I like quite a lot. There is
also a STATA theme, if you like it:
...
Even an Excel 2003 one (don't use it though):
...
### Color schemes from `ggthemes`

You can also change color schemes, by specifying either `scale_color_*` or `scale_fill_*`
functions. A color scheme I like is the [Highcharts](https://www.highcharts.com/) color scheme.
