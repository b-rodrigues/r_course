...
we are going to work a little bit with. `factor` variables are very useful, and the `forcats` package
includes some handy functions to work with `factor`s. First, let's load the `forcats` package:
...
`factor` variables have different levels, using `forcats::fct_recode()` you can recode levels:
