...
which is equal to `r booted_rmse_lin_lin`.

For the log-lin model, this is a bit more complicated, because
we need to exponentiate the predictions. However, if we use `rmse()`
as before, there is no way to do that. I show you the code below,
and try to understand it. You can see this as an advanced exercise;
if you understand these next lines of code, you should understand
anything that has to do with the `tidyverse`.


```{r}
booted_rmse_log_lin = cv_Housing %>%
...
`booted_rmse_log_lin` is equal to `r booted_rmse_log_lin`, which is lower than in the lin-lin model.
